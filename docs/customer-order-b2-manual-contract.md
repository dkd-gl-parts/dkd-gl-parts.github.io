...
- 注文履歴では、12桁の商品発送送り状番号を4桁ずつ区切って表示する。商品明細とコア代金は別行にし、商品小計、コア代金、送料、消費税、合計を注文詳細として表示する。
- コア代金請求済みの注文は、コア返却便を「未使用」、コア返却用送り状を「未発行」、コア返却を「請求済み」と表示する。後日返却する注文の`awaiting_return`は「要コア返却」と表示し、内部状態値を画面へ直接表示しない。
