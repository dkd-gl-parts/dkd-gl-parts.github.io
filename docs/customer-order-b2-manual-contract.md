...
- 受注時に交換コアを返却する運用ではない。商品マスタでコア返却必要の商品は、社内のFAX・メール注文入力で「後日、交換したコアを返却する」または「コアを返却できないためコア代金を支払う」を選択する。
- 後日返却を選んだ受注だけ、コア返却便、返送用送り状、コア返却管理の対象にする。
- コアを返却できない場合は、ブラウザから送られた金額を使用せず、商品マスタの返却不可時コア代金をサーバー側で数量分計上する。商品単価・商品計とは別の「コア代金」項目で表示し、消費税と合計には含める。コア返却状態は「請求済み」とし、返送用送り状を発行しない。
- コア代金を計上しても商品マスタの「コア返却必要」は変更しない。終了するのはその受注に対する後日の返却義務だけとする。
...
- `target_core_return_shipping_method`は後日返却する商品を含む場合だけ必須とし、`carrier_name`と`service_name`を含む。商品マスタで返却不要の商品だけの受注、または返却できないためコア代金を計上した受注ではnullを要求する。サーバーは発送便と別に検証し、送料マスタの有効便または返却専用の`佐川急便 / 飛脚宅配便`だけを許可する。
...
- コア返却便を変更した場合も確認済み状態を破棄するが、商品発送のお届け希望日は変更しない。商品マスタで返却不要の商品だけの受注と、コア代金請求済みの受注ではコア返却便を表示・保存しない。
- コア返却必要商品で「コアを返却できない」を選ぶと、商品マスタの返却不可時コア代金がサーバー側で数量分だけ計上され、商品代とは別の「コア代金」項目と「コア代金請求済み」を表示する。販売王CSVでも商品本体行の直後にコア代金行を分けて出力する。商品マスタで返却不要の商品にはコア代金が発生しない。
