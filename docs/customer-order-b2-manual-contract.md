...
8. 対象注文を選択し、`get_sales_order_b2_export`からB2基本レイアウトCSVを出力する。
9. B2クラウドへCSVを取り込み、出荷用と、必要な場合はコア返送用の送り状を発行する。
10. `register_sales_order_shipping`で出荷・返送の送り状番号を別々に登録する。
11. 出荷、コア返却受付、完了まで履歴を残す。
...
- D-CATSからB2へ出力する際、`お客様管理番号`へ商品発送は`注文番号-O`、コア返却は`注文番号-R`を設定する。
...
- 商品発送行だけが注文を出荷済みに進めて在庫引当を消費する。コア返却行は返却用送り状の発行履歴だけを登録し、コア返却完了にはしない。
