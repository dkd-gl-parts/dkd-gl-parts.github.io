...
- frontend `v1.1.696`は、`get_customer_order_feature_status`と得意先別公開設定の両方が有効になるまで実得意先の注文機能を表示しない。
...
- 郵便番号検索はzipcloudの郵便番号検索APIへ7桁の郵便番号だけを送信する。氏名、電話番号、会社名、住所は外部へ送信しない。検索不能時は手入力を継続できる。
