...
- 注文履歴では、商品発送とコア返却の帳票欄をどちらも「送り状番号」とし、12桁の送り状番号を4桁ずつ区切って表示する。使用しない運送便と送り状番号は「-」と表示する。
- 注文詳細では、商品とコア代金を別行にし、送料は商品・コア代金と同じ明細表へ独立行として表示する。商品小計、コア代金、消費税、合計は明細表の下に表示する。
- コア代金請求済みの注文は、コア返却を「請求済み」と表示する。後日返却する注文の`awaiting_return`は「要コア返却」と表示し、内部状態値を画面へ直接表示しない。取消注文の配送・返却情報は使用済みと誤認させない表示にする。
...
- 郵便番号検索は通常、日本郵便の公開データから生成した同一オリジンの端末内住所データを先に検索する。端末内データが利用できない場合または該当がない場合だけ、zipcloudの郵便番号検索APIへ7桁の郵便番号を送信する。氏名、電話番号、会社名、住所は外部へ送信しない。
- 注文画面を開いた後に10分割の端末内住所データをバックグラウンドでCache Storageへ保存し、画面を止めない。端末内検索と外部fallbackの両方が失敗しても、住所の手入力は継続できる。
- 「自動」「APIのみ」「ローカルのみ」の切り替えは、社内の「受注・出荷管理 > データ連携 > 住所検索設定」にだけ表示する。得意先注文画面には切替UIとデータ源を表示しない。
- ローカル住所データはGitHub Actionsで毎月、日本郵便のUTF-8全国一括データから再生成する。件数、47都道府県、代表郵便番号、分割ファイルのSHA-256検証を通過した変更は、14日保持の更新候補artifactとして出力し、repositoryへ直接commit/pushしない。
- APIとローカルデータの両方で検索不能な場合も住所の手入力を継続できる。通信断中は住所変換だけが可能で、注文確定はサーバー接続回復後に行う。
- 商品発送便は有効な契約送料マスタから選択する。添付配送マスタの60サイズ定価は参考値として非公開領域に保持し、受注送料へ使用しない。コア返却便は返却必要商品を含む場合だけ表示し、送料マスタの有効便に加えて返却専用の`佐川急便 / 飛脚宅配便`を選択できる。
- 発地`590-0121`の郵便番号別配送マスタをサーバー側の正本とし、`get_customer_order_delivery_quote`で郵便番号完全一致、または住所の市区町村一致から最短日と時間帯制約を返す。住所一致は同一市区町村内で最も遅い条件を採り、ブラウザ計算を信用しない。
- 宅急便系は`発送日 + lead_days`をお届け希望日へ自動設定し、得意先は発送日から13日後までの範囲内で変更できる。ネコポス、クロネコゆうパケット、クロネコゆうメールなど日時指定不可のサービスは、希望日・時間帯を送信せず到着目安だけを表示する。
- `customer_orders.scheduled_shipping_date`を注文ごとの発送予定日の正本とする。受注時は日本時間16:59:59まで当日、17:00:00以降は翌日の暦日を保存し、土日も飛ばさない。
- 受注画面の発送予定日はサーバー計算結果を読み取り専用で表示し、得意先は過去日や任意日へ変更できない。受注・出荷管理では未出荷注文に限り、当日または未来日へ変更できる。
- 受注・出荷管理で発送予定日を変更した場合は配送制約を再検証し、既発行のB2 CSV・商品発送用送り状を無効化して再発行を要求する。B2 CSVは保存済みの発送予定日を使用し、過去日なら出力を拒否する。
...
9. CSVデータ破損・紛失で同じ内容が必要な場合は発行時のデータを再ダウンロードする。送り状の破損・紛失、受注修正後の再印刷など、内容を再作成する場合だけ定型理由と補足を記録して再発行する。
...
- 商品マスタでコア返却必要の商品は、社内のFAX・メール注文入力で「後日、交換したコアを返却する」または「コア代金を請求し、返却受付後に返金する」を選択する。どちらも交換コアの返却対象とする。
- コア代金を請求した受注も、コア返却便、返送用送り状、コア返却管理の対象にする。
- コア代金を請求する場合は、ブラウザから送られた金額を使用せず、商品マスタのコア代金をサーバー側で数量分計上する。商品単価・商品計とは別の「コア代金」項目で表示し、消費税と合計には含める。返却前の表示は「請求済み」とする。
- 交換コアを受け付けた後は「返却済み・返金確認」とし、返金処理を記録した後は「返金済み」とする。返金額、処理日時、処理者、メモをサーバー側の監査対象として保存する。
- コア代金を計上しても商品マスタと受注明細の「コア返却必要」は変更しない。後日の返却義務は、コア返却受付まで継続する。
- 商品マスタでコア返却不要の商品には選択欄を表示しない。コア代金は設定・計上せず、不正な計上指定はサーバー側で拒否する。
...
- `target_core_return_shipping_method`は後日返却する商品を含む場合に必須とし、`carrier_name`と`service_name`を含む。コア代金を請求した受注も返却対象に含める。商品マスタで返却不要の商品だけの受注ではnullを要求する。サーバーは発送便と別に検証し、送料マスタの有効便または返却専用の`佐川急便 / 飛脚宅配便`だけを許可する。
- `product_core_return_required`は商品マスタの条件、`core_return_required`はその受注で後日返却が残っているか、`core_charge_billed`は商品マスタのコア代金を受注時に計上したかを表す。この3つを混同しない。
- 商品発送便とコア返却便を注文スナップショットへ別々に保存する。B2 CSVは商品発送便だけに使用し、コア返却は複写伝票管理に使用する。
- `preview_token`は短時間のみ有効とし、ユーザー、得意先、明細、価格、在庫確認時刻へ結び付ける。

### `get_customer_order_delivery_quote(target_postal_code text, target_address text, target_carrier_name text, target_service_name text, target_shipping_date date)`

- 認証済み得意先または受注管理者だけが実行できる読み取り専用RPCとする。
- 郵便番号完全一致を優先し、郵便番号がない場合は都道府県＋市区町村までの住所で保守的に解決する。
- 日本時間16:59:59までは受注日の暦日、17:00:00以降は翌日の暦日を発送日とし、土日を飛ばさず`発送日 + lead_days`を最短着日にする。
- frontendはRPCが返す`shipping_date`を「発送予定日」の読み取り専用日付欄へ表示し、「お届け希望日」の直上に配置する。
- お届け希望日の上限は発送日から13日後とし、14日以上先は返さない。
- ヤマトの`AM`、`T14`、`T18`、`NONE`から利用可能時間帯コードを返す。佐川は時間帯の正本がないため時間指定を許可しない。
- 離島警告、推定精度、市区町村の曖昧解決を返して画面に表示する。
- 契約送料の有無は既存`customer_shipping_rates`で確認し、正確な契約送料は商品サイズ・重量を持つ受注プレビューで確定する。配送マスタの定価は使用しない。
...
### `update_sales_order_scheduled_shipping_date(target_order_id bigint, target_scheduled_shipping_date date, target_expected_version bigint)`

- 社内の受注管理権限を持つユーザーだけが実行できる。
- `submitted`、`accepted`、`shipping_ready`の未出荷注文に限り、発送予定日を当日または未来日へ変更できる。前日以前には変更できない。
- versionを確認して同時更新を防ぎ、変更後の発送予定日で最短着日・14日制限・時間帯を再検証する。
- 17時以降に翌日で受注された注文も、実際に当日出荷できる場合は管理画面から当日へ戻せる。
- 既発行のB2 CSV、商品発送用送り状、関連印刷待ちを無効化し、変更前後と操作者を監査履歴へ残す。
...
- B2作成時の実際の出荷予定日を基準に、最短着日より前、発送日から14日以上先、郵便番号で許可されない時間帯、日時指定不可サービスをエラーにして発行を止める。
- B2の出荷予定日は`customer_orders.scheduled_shipping_date`を使用し、当日より前の日付が残っている注文は管理画面で更新するまで発行を止める。
...
- コア返却便を変更した場合も確認済み状態を破棄するが、商品発送のお届け希望日は変更しない。商品マスタで返却不要の商品だけの受注ではコア返却便を表示・保存しない。コア代金請求済みの受注は返却対象なので表示・保存する。
- コア返却必要商品で「コア代金を請求」を選ぶと、商品マスタのコア代金がサーバー側で数量分だけ計上され、商品代とは別の「コア代金」項目と「請求済み」を表示する。販売王CSVでも商品本体行の直後にコア代金行を分けて出力する。商品マスタで返却不要の商品にはコア代金が発生しない。
- お届け希望日のブラウザ改変ではサーバー算出の最短日より前を指定できず、日時指定不可サービスへ希望日・時間帯を保存できない。
- お届け希望日のブラウザ改変では発送日から13日後を超えて指定できず、同じ検証が受注保存とB2出力の両方で働く。
- 発送予定日は受注画面で変更できず、受注・出荷管理では未出荷注文だけを当日または未来日へ変更できる。17時以降に翌日で受注された注文を当日に戻すと、着日制約とB2出力が当日発送基準で再計算される。
- API認証キーはpublic frontend、Git、operation logへ残らない。
- Security Advisorに新しい未解決警告がない。

## 郵便データ更新候補の昇格手順

1. workflow summaryで「変更なし」または候補artifact名・14日の期限・次アクションを確認する。
2. GitHubで認証して取得したartifactであっても、信頼済みsourceやそのまま配信できる成果物とは扱わない。対象runのcommit、workflow、Japan Postのsource URLとmanifestを確認する。
3. 最新のreview済み`main`からclean branch/worktreeを作り、日本郵便の公式UTF-8全国一括データを改めて取得して`update-postal-data.js`で再生成する。artifactのmanifest、各shardのSHA-256、件数、47都道府県、代表郵便番号と再生成結果を比較する。
4. `verify-postal-data.js`、`verify-postal-lookup-fallback.js`、`verify-workflow-coverage.js`、全frontend verifier、JavaScript構文、static build、response header、diff/secret検査を再実行する。不一致や期限切れartifactは破棄し、推測で補完しない。
5. 再生成した差分だけを通常のreview付きPRへ載せる。workflow token、PAT、`contents: write`、scheduled direct pushで昇格させない。merge後は通常のfrontend release監視とproductionのversion/hash確認を行う。
