...
- コアを返却できない場合は、ブラウザから送られた金額を使用せず、商品マスタの返却不可時コア代金をサーバー側で商品単価、商品計、消費税、合計へ加算する。受注明細は「コア代金請求済み」とし、返送用送り状を発行しない。
...
- コア返却必要商品で「コアを返却できない」を選ぶと、商品マスタの返却不可時コア代金がサーバー側で数量分だけ計上され、「コア代金請求済み」と表示される。商品マスタで返却不要の商品にはコア代金が発生しない。
