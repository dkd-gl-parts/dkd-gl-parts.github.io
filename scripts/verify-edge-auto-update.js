...
async function verifyNetworkCheck() {
  ...
  const sandbox = {
    ...
    fetch: async (url, options) => {
      ...
      return { ok: true, text: async () => '<meta name="dcats-app-version" content="v1.1.795">' };
    },
    DOMParser: class {
      parseFromString() {
        return { querySelector: () => ({ getAttribute: () => "v1.1.795" }) };
      }
    },
  ...
  if (!sandbox.appUpdateDetected || scheduledVersion !== "v1.1.795" || sandbox.appUpdateCheckInProgress) {
