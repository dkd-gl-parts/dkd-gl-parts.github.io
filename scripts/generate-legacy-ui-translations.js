...
const curatedTranslations = {
  en: {
    ...
    "ログイン後、下のボタンから「発行済データの検索」を直接開きます。": "After signing in, use the button below to open Issued Data Search directly.",
    "発行済データをCSV出力": "Export issued data as CSV",
    "「発行済データの検索」で対象を検索し、「外部ファイル出力」でCSVをダウンロードします。": "Find the target records under Issued Data Search, then download the CSV using External File Export.",
    ...
    "2 発行済データの検索を開く": "2 Open Issued Data Search",
    ...
    "B2取込済み": "B2 Import Complete",
    "B2取込待ち": "Awaiting B2 Import",
  ...
  zh: {
    ...
    "ログイン後、下のボタンから「発行済データの検索」を直接開きます。": "登录后，使用下方按钮直接打开“搜索已发行数据”。",
