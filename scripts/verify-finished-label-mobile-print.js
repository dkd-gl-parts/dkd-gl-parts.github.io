...
assert(appVersionMatch, "app version is missing");
assert(html.includes(`content="${appVersionMatch[1]}"`), "HTML version does not match the app version");
...
assert(functionSource("requestedFinishedLabelPrintStationTarget").includes("dcats_print_station") && functionSource("openRequestedFinishedLabelPrintStation").includes("prepareFinishedLabelCompatibilityStation()") && functionSource("prepareFinishedLabelCompatibilityStation").includes("resumeFinishedLabelPrintStationIfEnabled()"), "dedicated Windows station does not auto-start receiving");
