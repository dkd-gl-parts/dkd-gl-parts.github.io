...
assert(!panel.hidden && panel.classList.contains("is-question-view"), "A short character click did not open the dedicated question view");
assert(documentObject.activeElement === aiQuestionInput, "Character question view did not focus the question input");
assert(hitTarget.getAttribute("aria-expanded") === "true" && launcher.getAttribute("aria-expanded") === "false", "Character question view exposed the wrong expanded trigger");
assert(mover.getBoundingClientRect().left <= 10, "Character did not move beside the question panel");
dispatch(panelClose.listeners, "click", { target: panelClose });
assert(panel.hidden && documentObject.activeElement === hitTarget, "Closing the question view did not restore character focus");
launcher.focus();
dispatch(launcher.listeners, "click", { target: launcher });
assert(!panel.hidden && !panel.classList.contains("is-question-view"), "Header launcher did not preserve the settings view");
assert(documentObject.activeElement === panelClose && launcher.getAttribute("aria-expanded") === "true", "Settings view did not focus its close control");
dispatch(panelClose.listeners, "click", { target: panelClose });
...
const bridgeFolderButton = bridgeButtons.find((element) => element.textContent === "取込待ちフォルダーを開く");
const bridgeLaunchButton = bridgeButtons.find((element) => element.textContent === "販売王25を起動");
...
bridgeResponseFactory = (request) => ({
  ...
  data: { reused: false, validation: { summary: { rowCount: 2, slipCount: 1 }, warnings: [] } }
});
...
bridgeResponseFactory = (request) => ({
  ...
  ok: true,
  command: request.command,
  data: { target: "test_company_import_folder", opened: true }
});
dispatch(bridgeFolderButton.listeners, "click", { target: bridgeFolderButton });
await new Promise((resolve) => setImmediate(resolve));
await new Promise((resolve) => setImmediate(resolve));
assert(capabilityRequests.length === 4 && capabilityRequests[3].command === "open_test_company_import_folder" && capabilityRequests[3].args === undefined, "Fixed folder launch accepted browser-supplied arguments");
assert(bridgeRequests.length === 4 && bridgeRequests[3].args === undefined && bridgeRequests[3].capability === "signed-capability", "Fixed folder launch was not forwarded safely");
assert(bridgeStatus.textContent.includes("取込待ちフォルダーを開きました") && bridgeStatus.textContent.includes("手動で行ってください") && bridgeStatus.classList.contains("is-success"), "Fixed folder launch did not preserve the manual-import safety message");
bridgeResponseFactory = (request) => ({
  id: request.id,
  ok: true,
  command: request.command,
  data: { target: "hanbaioh25", launched: false, alreadyRunning: true }
});
dispatch(bridgeLaunchButton.listeners, "click", { target: bridgeLaunchButton });
await new Promise((resolve) => setImmediate(resolve));
await new Promise((resolve) => setImmediate(resolve));
assert(capabilityRequests.length === 5 && capabilityRequests[4].command === "launch_hanbaioh25" && capabilityRequests[4].args === undefined, "Sales King launch accepted browser-supplied arguments");
assert(bridgeRequests.length === 5 && bridgeRequests[4].args === undefined && bridgeRequests[4].capability === "signed-capability", "Sales King launch was not forwarded safely");
assert(bridgeStatus.textContent.includes("販売王25はこのPCで起動済みです") && bridgeStatus.textContent.includes("別ログインは行いません") && bridgeStatus.textContent.includes("ユーザー名") && bridgeStatus.textContent.includes("データ名称") && bridgeStatus.textContent.includes("取込を止めて") && !bridgeStatus.textContent.includes("D-CATS連携テスト（実データ禁止）") && bridgeStatus.classList.contains("is-warning") && !bridgeStatus.classList.contains("is-success"), "An already-running Sales King session must require identity and company review before import");
assert(byClass("dcats-concierge-bubble")[0].textContent === "販売王は起動中です。利用者と会社データを確認してください。", "An already-running Sales King session must show a short character bubble");
documentObject.documentElement.lang = "en";
notifyObservers();
assert(bridgeStatus.textContent.includes("No second sign-in") && bridgeStatus.textContent.includes("stop the import"), "English already-running guidance was not translated");
documentObject.documentElement.lang = "zh-CN";
notifyObservers();
assert(bridgeStatus.textContent.includes("不会再次登录") && bridgeStatus.textContent.includes("停止导入"), "Chinese already-running guidance was not translated");
documentObject.documentElement.lang = "ja";
notifyObservers();
bridgeSalesFileInput.value = "..\\sales.csv";
dispatch(bridgeSalesButton.listeners, "click", { target: bridgeSalesButton });
assert(capabilityRequests.length === 5 && bridgeStatus.textContent === "受信フォルダー内のCSVファイル名だけを入力してください。", "Unsafe sales CSV file name was not rejected before capability issuance");
bridgeSalesFileInput.value = "sales.csv";
bridgeResponseFactory = (request) => ({
  id: request.id,
  ok: true,
  command: request.command,
  data: { reused: false, validation: { summary: { rowCount: 3, uniqueCustomerCount: 2 }, warnings: [{ code: "DUPLICATE_CUSTOMER" }] } }
});
dispatch(bridgeCustomerButton.listeners, "click", { target: bridgeCustomerButton });
await new Promise((resolve) => setImmediate(resolve));
await new Promise((resolve) => setImmediate(resolve));
assert(capabilityRequests.length === 6 && capabilityRequests[5].command === "prepare_customer_import" && capabilityRequests[5].args.fileName === "customers.csv", "Customer CSV preparation was not bound to the reviewed file name");
...
await new Promise((resolve) => setImmediate(resolve));
await new Promise((resolve) => setImmediate(resolve));
assert(bridgeStatus.textContent === "CSVの事前検査で修正箇所が見つかりました：項目数は56ですが、55項目です。" && bridgeStatus.classList.contains("is-error"), "CSV validation failure was not presented safely");
