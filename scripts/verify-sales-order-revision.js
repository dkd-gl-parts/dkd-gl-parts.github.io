...
assert.equal(Object.keys(context.SALES_ORDER_REVISION_ENTRY_FIELDS).length,23);
assert.equal(context.SALES_ORDER_REVISION_ENTRY_FIELDS["customer-order-destination-type"],"destination_type");
assert.equal(context.SALES_ORDER_REVISION_ENTRY_FIELDS["customer-order-yamato-office-code"],"yamato_office_selection");
assert.equal(context.SALES_ORDER_REVISION_ENTRY_FIELDS["customer-order-yamato-office-custom-code"],"yamato_office_custom_code");
assert.equal(context.SALES_ORDER_REVISION_ENTRY_FIELDS["customer-order-yamato-office-custom-name"],"yamato_office_custom_name");
...
assert(source.includes('values.yamato_office_selection = savedYamatoOffice'));
assert(source.includes('salesOrderRevisionValue("yamato_office_custom_code")'));
assert(source.includes('salesOrderRevisionValue("yamato_office_custom_name")'));
assert(source.includes('document.getElementById("revision-entry-yamato-office-custom")'));
...
const revisionLabels={core_return_required:'コア返却必要',customer_order_core_handling:'交換コアの対応',customer_order_core_return_standard:'後日、交換したコアを返却する',customer_order_core_charge_no_return_label:'コア代金 {amount} 請求済み・要返却',customer_order_core_charge_no_return_status:'コア代金請求済み',customer_order_core_charge_unset:'コア代金が未設定（選択不可）',customer_order_core_charge_setup:'商品マスタにコア代金を設定してください。',customer_order_core_charge_note:'商品マスタのコア代金を商品代とは別項目で請求します。返送用送り状を発行し、コア返却受付後に返金します。',customer_order_core_charge_total:'コア代金',customer_order_unit_price:'商品単価',customer_order_line_total:'明細計'};
...
assert(context.salesOrderRevisionCoreChoiceHtml(noCharge,1).includes('コア代金が未設定（選択不可）'));
