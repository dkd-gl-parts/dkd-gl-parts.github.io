...
assert(source.includes("salesOrderShippingScheduleHtml(order)"), "sales-order detail must render the shipping-date editor with delivery details");
const salesOrderDetail = sourceBetween("function renderSalesOrderDetail", "async function openSalesOrderSerialWarranty");
assert(salesOrderDetail.indexOf("salesOrderDestinationHtml(address)") < salesOrderDetail.indexOf('"<dl>" + deliveryFacts'), "the destination must precede the outbound shipping facts");
assert(salesOrderDetail.indexOf('"<dl>" + deliveryFacts') < salesOrderDetail.indexOf("salesOrderShippingScheduleHtml(order)"), "the outbound shipping facts must appear immediately left of the shipping-date editor on desktop");
